 ...
 */
define('USER1_SELECTOR_DEFAULT_ROWS', 20);
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class user1_selector_base {
    ...
    protected $rows = USER1_SELECTOR_DEFAULT_ROWS;
    ...
    protected static $jsmodule = array(
                'name' => 'user_selector',
                'fullpath' => '/local/vilt/selector/module.js',
    ...
    public function display($return = false) {
        ...
        // $groupedusers = $DB->get_record('user', [], );
        ...
        $optionsoutput = false;
        if (!user1_selector_base::$searchoptionsoutput) {
            ...
            user1_selector_base::$searchoptionsoutput = true;
        }
        $output .= "</div>\n</div>\n\n";
    ...
    protected function required_fields_sql($u) {
        ...
        $fields = array_merge($fields, \core_user\fields::for_name()->get_required_fields(), $this->extrafields);
    ...
    protected function output_optgroup($groupname, $users, $select) {
        ...
        if (!empty($users)) {
            ...
            foreach ($users as $user) {
                $lpdisable = '';$msg ='';
                if($groupname == 'Potential users'){
                    // if($DB->record_exists('lp_members', array('userid' => $user->id))) {
                    //    $lpdisable = 1;
                    // }
                }
                $attributes = '';
                if (!empty($user->disabled)) {
                    ...
                    $attributes .= ' disabled="disabled" style="color:#6e211e;background-color:#f6d9d8;"';
                    $msg = '- Already assigned ';
                }
                unset($this->selected[$user->id]);
                $output .= '    <option' . $attributes . ' value="' . $user->id . '">' .
                        $this->output_user($user) . $msg ."</option>\n";
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class groups_user1_selector_base extends user1_selector_base {
    /** @var int */
    protected $groupid;
    /** @var  int */
    protected $courseid;
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class group_members1_selector extends groups_user1_selector_base {
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class group_non_members1_selector extends groups_user1_selector_base {
    /**
