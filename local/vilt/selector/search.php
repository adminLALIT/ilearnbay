...
$PAGE->set_url('/local/vilt/selector/search.php');
...
$name = $options['name'];
unset($options['name']);
$options['companyid'] = $_SESSION['companyid'];
...
foreach ($results as $groupname => $users) {
    ...
    foreach ($users as $user) {
        $lpdisable = '';$msg ='';
                if($groupname == 'Potential users' || $groupname == 'Potential matching users'){
                    if($DB->record_exists('webexmeeting_members', array('userid' => $user->id))) {
                       $lpdisable = 1;
                    }
                }
        $output = new stdClass;
        ...
        if($lpdisable == 1){
            $output->disabled = true;
            $output->name = $userselector->output_user($user) . '- Already assigned ';
        }
